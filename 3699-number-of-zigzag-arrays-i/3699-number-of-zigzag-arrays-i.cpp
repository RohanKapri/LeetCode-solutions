...
/* dbg(n, arr, goal); prints [n, arrr, goal]: */
#endif
...
template <class C, class T, class Comp> 
inline long long LB(const C&c, const T&x, Comp comp) {
    return lower_bound(c.begin(), c.end(), x, comp) - c.begin();
}
template <class C, class T>
inline long long UB(const C &c, const T &x) {
    return upper_bound(c.begin(), c.end(), x) - c.begin(); /* span(first, last) */
}
template <class C, class T, class Comp>
inline long long UB(const C &c, const T &x, Comp comp) {
    return upper_bound(c.begin(), c.end(), x, comp) - c.begin();
}
...
#define ROTATE(v, k) rotate((v).begin(), (v).begin() + (((k) % len(v) + len(v)) % len(v)), (v).end())
#define RROTATE(v, k) rotate((v).rbegin(), (v).rbegin() + (((k) % len(v) + len(v)) % len(v)), (v).rend())
...
template<typename T, typename U>
vc<pair<T, U>> ZIP(const vc<T> &A, const vc<T> &B) {
    vc<pair<T, U>> res(min(len(A), len(B)));
    rep(i, len(res)) res[i] = {A[i], B[i]};
    return res;
}
...
template<int MOD>
istream& operator>>(istream& is, modint<MOD>& o) { long long t; is >> t; o = modint<MOD>(t); return is; }
template<int MOD>
ostream& operator<<(ostream& os, const modint<MOD>& o) { return os << o.v; }

using mint = modint<1000000007>;
// using mint = modint<998244353>;
// using mint = modint; // mint::set_mod(m)
// using mint = modint<SET_VALUE>; /* /= and inv() only work if MOD is prime. */
